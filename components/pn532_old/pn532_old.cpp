...
static const char *TAG = "pn532_old";
...
void PN532::loop() {
  ...
  if (next_task_ == READ) {
    ...
    if (report) {
      ESP_LOGD(TAG, "Found new tag '%s'", nfc_old::format_uid(nfcid).c_str());
...
nfc_old::NfcTag *PN532::read_tag_(std::vector<uint8_t> &uid) {
  uint8_t type = nfc_old::guess_tag_type(uid.size());

  if (type == nfc_old::TAG_TYPE_MIFARE_CLASSIC) {
    ESP_LOGD(TAG, "Mifare classic");
    return this->read_mifare_classic_tag_(uid);
  } else if (type == nfc_old::TAG_TYPE_2) {
    ESP_LOGD(TAG, "Mifare ultralight");
    return this->read_mifare_ultralight_tag_(uid);
  } else if (type == nfc_old::TAG_TYPE_UNKNOWN) {
    ESP_LOGV(TAG, "Cannot determine tag type");
    return new nfc_old::NfcTag(uid);
  } else {
    return new nfc_old::NfcTag(uid);
  }
}
...
void PN532::write_mode(nfc_old::NdefMessage *message) {
...
bool PN532::write_tag_(std::vector<uint8_t> &uid, nfc_old::NdefMessage *message) {
  uint8_t type = nfc_old::guess_tag_type(uid.size());
  if (type == nfc_old::TAG_TYPE_MIFARE_CLASSIC) {
...
void PN532OnTagTrigger::process(nfc_old::NfcTag *tag) { this->trigger(nfc_old::format_uid(tag->get_uid()), *tag); }
