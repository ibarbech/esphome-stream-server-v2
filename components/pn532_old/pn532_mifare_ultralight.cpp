...
static const char *TAG = "pn532_old.mifare_ultralight";

nfc_old::NfcTag *PN532::read_mifare_ultralight_tag_(std::vector<uint8_t> &uid) {
  if (!this->is_mifare_ultralight_formatted_()) {
    ESP_LOGD(TAG, "Not NDEF formatted");
    return new nfc_old::NfcTag(uid, nfc_old::NFC_FORUM_TYPE_2);
  }
  ...
  uint8_t index = 0;
  for (uint8_t page = nfc_old::MIFARE_ULTRALIGHT_DATA_START_PAGE; page < nfc_old::MIFARE_ULTRALIGHT_MAX_PAGE; page++) {
  ...
  return new nfc_old::NfcTag(uid, nfc_old::NFC_FORUM_TYPE_2, data);
}
...
bool PN532::read_mifare_ultralight_page_(uint8_t page_num, std::vector<uint8_t> &data) {
  ...
  ESP_LOGVV(TAG, "Pages %d-%d: %s", page_num, page_num + 4, nfc_old::format_bytes(data).c_str());
...
bool PN532::write_mifare_ultralight_tag_(std::vector<uint8_t> &uid, nfc_old::NdefMessage *message) {
  uint32_t capacity = this->read_mifare_ultralight_capacity_();

  auto encoded = message->encode();

  uint32_t message_length = encoded.size();
  uint32_t buffer_length = nfc_old::get_mifare_ultralight_buffer_size(message_length);
  ...
  uint32_t index = 0;
  uint8_t current_page = nfc_old::MIFARE_ULTRALIGHT_DATA_START_PAGE;

  while (index < buffer_length) {
    std::vector<uint8_t> data(encoded.begin() + index, encoded.begin() + index + nfc_old::MIFARE_ULTRALIGHT_PAGE_SIZE);
    if (!this->write_mifare_ultralight_page_(current_page, data)) {
      return false;
    }
    index += nfc_old::MIFARE_ULTRALIGHT_PAGE_SIZE;
    current_page++;
  }
  return true;
}
...
bool PN532::clean_mifare_ultralight_() {
  uint32_t capacity = this->read_mifare_ultralight_capacity_();
  uint8_t pages = (capacity / nfc_old::MIFARE_ULTRALIGHT_PAGE_SIZE) + nfc_old::MIFARE_ULTRALIGHT_DATA_START_PAGE;

  std::vector<uint8_t> blank_data = {0x00, 0x00, 0x00, 0x00};

  for (int i = nfc_old::MIFARE_ULTRALIGHT_DATA_START_PAGE; i < pages; i++) {
...
bool PN532::write_mifare_ultralight_page_(uint8_t page_num, std::vector<uint8_t> &write_data) {
  std::vector<uint8_t> data({
      PN532_COMMAND_INDATAEXCHANGE,
      0x01,  // One card
      nfc_old::MIFARE_CMD_WRITE_ULTRALIGHT,
      page_num,
  });
