...
static const char *TAG = "pn532_old.mifare_classic";

nfc_old::NfcTag *PN532::read_mifare_classic_tag_(std::vector<uint8_t> &uid) {
  uint8_t current_block = 4;
  uint8_t message_start_index = 0;
  uint32_t message_length = 0;

  if (this->auth_mifare_classic_block_(uid, current_block, nfc_old::MIFARE_CMD_AUTH_A, nfc_old::NDEF_KEY)) {
    std::vector<uint8_t> data;
    if (this->read_mifare_classic_block_(current_block, data)) {
      if (!nfc_old::decode_mifare_classic_tlv(data, message_length, message_start_index)) {
        return new nfc_old::NfcTag(uid, nfc_old::ERROR);
      }
    } else {
      ESP_LOGE(TAG, "Failed to read block %d", current_block);
      return new nfc_old::NfcTag(uid, nfc_old::MIFARE_CLASSIC);
    }
  } else {
  ...
  uint32_t index = 0;
  uint32_t buffer_size = nfc_old::get_mifare_classic_buffer_size(message_length);
  std::vector<uint8_t> buffer;

  while (index < buffer_size) {
    if (nfc_old::mifare_classic_is_first_block(current_block)) {
      if (!this->auth_mifare_classic_block_(uid, current_block, nfc_old::MIFARE_CMD_AUTH_A, nfc_old::NDEF_KEY)) {
    ...
    index += nfc_old::MIFARE_CLASSIC_BLOCK_SIZE;
    current_block++;

    if (nfc_old::mifare_classic_is_trailer_block(current_block)) {
      current_block++;
    }
  }
  buffer.erase(buffer.begin(), buffer.begin() + message_start_index);
  return new nfc_old::NfcTag(uid, nfc_old::MIFARE_CLASSIC, buffer);
}
...
bool PN532::read_mifare_classic_block_(uint8_t block_num, std::vector<uint8_t> &data) {
  ...
  ESP_LOGVV(TAG, " Block %d: %s", block_num, nfc_old::format_bytes(data).c_str());
  return true;
}
...
bool PN532::format_mifare_classic_mifare_(std::vector<uint8_t> &uid) {
  ...
  for (int block = 0; block < 64; block += 4) {
    if (!this->auth_mifare_classic_block_(uid, block + 3, nfc_old::MIFARE_CMD_AUTH_B, nfc_old::DEFAULT_KEY)) {
      continue;
    }
    if (block != 0) {
...
bool PN532::format_mifare_classic_ndef_(std::vector<uint8_t> &uid) {
  ...
  if (!this->auth_mifare_classic_block_(uid, 0, nfc_old::MIFARE_CMD_AUTH_B, nfc_old::DEFAULT_KEY)) {
...
bool PN532::write_mifare_classic_tag_(std::vector<uint8_t> &uid, nfc_old::NdefMessage *message) {
  auto encoded = message->encode();

  uint32_t message_length = encoded.size();
  uint32_t buffer_length = nfc_old::get_mifare_classic_buffer_size(message_length);
  ...
  while (index < buffer_length) {
    ...
    std::vector<uint8_t> data(encoded.begin() + index, encoded.begin() + index + nfc_old::MIFARE_CLASSIC_BLOCK_SIZE);
