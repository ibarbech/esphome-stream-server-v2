...
from esphome.components import nfc_old
from esphome.const import CONF_ID, CONF_ON_TAG, CONF_TRIGGER_ID
from esphome.core import coroutine

CODEOWNERS = ['@OttoWinter', '@jesserockz']
AUTO_LOAD = ['binary_sensor', 'nfc_old']
MULTI_CONF = True

CONF_PN532_ID = 'pn532_id'
CONF_ON_FINISHED_WRITE = 'on_finished_write'

pn532_ns = cg.esphome_ns.namespace('pn532_old')
PN532 = pn532_ns.class_('PN532', cg.PollingComponent)

PN532OnTagTrigger = pn532_ns.class_('PN532OnTagTrigger',
                                    automation.Trigger.template(cg.std_string, nfc_old.NfcTag))
...
def setup_pn532(var, config):
    ...
    for conf in config.get(CONF_ON_TAG, []):
        ...
        yield automation.build_automation(trigger, [(cg.std_string, 'x'), (nfc_old.NfcTag, 'tag')],
                                          conf)
