...
#include "esphome/components/nfc_old/nfc_tag.h"
#include "esphome/components/nfc_old/nfc.h"
...
class PN532 : public PollingComponent {
 public:
  ...
  void clean_mode();
  void format_mode();
  void write_mode(nfc_old::NdefMessage *message);
 ...
 protected:
  ...
  nfc_old::NfcTag *read_tag_(std::vector<uint8_t> &uid);

  bool format_tag_(std::vector<uint8_t> &uid);
  bool clean_tag_(std::vector<uint8_t> &uid);
  bool write_tag_(std::vector<uint8_t> &uid, nfc_old::NdefMessage *message);

  nfc_old::NfcTag *read_mifare_classic_tag_(std::vector<uint8_t> &uid);
  ...
  bool write_mifare_classic_tag_(std::vector<uint8_t> &uid, nfc_old::NdefMessage *message);

  nfc_old::NfcTag *read_mifare_ultralight_tag_(std::vector<uint8_t> &uid);
  ...
  bool write_mifare_ultralight_tag_(std::vector<uint8_t> &uid, nfc_old::NdefMessage *message);
  ...
  nfc_old::NdefMessage *next_task_message_to_write_;
  enum NfcTask {
    READ = 0,
    CLEAN,
    FORMAT,
...
class PN532OnTagTrigger : public Trigger<std::string, nfc_old::NfcTag> {
 public:
  void process(nfc_old::NfcTag *tag);
};
